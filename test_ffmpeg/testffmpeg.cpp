...
static const GUID SDL_IID_ID3D11Resource = {0xdc8e63f3,
                                            0xd12b,
                                            0x4952,
...
static SDL_bool CreateWindowAndRenderer(SDL_WindowFlags window_flags, const char* driver)
{
    ...
    SDL_bool useOpenGL =
        (driver && (SDL_strcmp(driver, "opengl") == 0 || SDL_strcmp(driver, "opengles2") == 0));
    ...
    if (SDL_GetBooleanProperty(SDL_GetDisplayProperties(SDL_GetDisplayForWindow(window)),
    ...
    if (useEGL) {
        ...
        if (has_EGL_EXT_image_dma_buf_import && glEGLImageTargetTexture2DOESFunc &&
            glActiveTextureARBFunc) {
...
#ifdef SDL_PLATFORM_WIN32
    d3d11_device = (ID3D11Device*)SDL_GetProperty(SDL_GetRendererProperties(renderer),
...
static SDL_Texture* CreateTexture(SDL_Renderer* r,
                                  unsigned char* data,
                                  unsigned int len,
                                  int* w,
                                  int* h)
...
{
    if (!software_only) {
        if (has_eglCreateImage && (format == AV_PIX_FMT_VAAPI || format == AV_PIX_FMT_DRM_PRIME)) {
            return SDL_TRUE;
        }
#ifdef SDL_PLATFORM_APPLE
...
static enum AVPixelFormat GetSupportedPixelFormat(AVCodecContext* s,
...
static AVCodecContext* OpenVideoStream(AVFormatContext* ic, int stream, const AVCodec* codec)
{
    ...
    SDL_Log("Video stream: %s %dx%d\n", avcodec_get_name(codec->id), codecpar->width,
            codecpar->height);
    ...
    i = 0;
    while (!context->hw_device_ctx && (config = avcodec_get_hw_config(codec, i++)) != NULL) {
#if 0
        ...
        if (d3d11_device && config->device_type == AV_HWDEVICE_TYPE_D3D11VA) {
            ...
            device_context =
                (AVVulkanDeviceContext*)((AVHWDeviceContext*)context->hw_device_ctx->data)->hwctx;
            ...
            result =
                av_hwdevice_ctx_create(&context->hw_device_ctx, config->device_type, NULL, NULL, 0);
            if (result < 0) {
                SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                             "Couldn't create %s hardware device context: %s",
                             av_hwdevice_get_type_name(config->device_type), av_err2str(result));
...
static SDL_Colorspace GetFrameColorspace(AVFrame* frame)
{
    ...
    if (frame && frame->colorspace != AVCOL_SPC_RGB) {
        ...
        SDL_Log(
            "Frame colorspace: range: %d, primaries: %d, trc: %d, colorspace: %d, chroma_location: "
            "%d\n",
            frame->color_range, frame->color_primaries, frame->color_trc, frame->colorspace,
            frame->chroma_location);
#endif
        colorspace = static_cast<SDL_Colorspace>(
            SDL_DEFINE_COLORSPACE(SDL_COLOR_TYPE_YCBCR, frame->color_range, frame->color_primaries,
                                                     ...
                                                     int access)
{
    ...
    /* ITU-R BT.2408-6 recommends using an SDR white point of 203 nits, which is more likely for
     * game content */
    ...
    if (frame->hw_frames_ctx) {
        ...
        if (format == SDL_PIXELFORMAT_UNKNOWN) {
            format = GetTextureFormat(static_cast<AVPixelFormat>(frame->format));
        }
    }
    ...
    if (pSideData) {
        ...
        /* The official definition is 10000, but PQ game content is often mastered for 400 or 1000
         * nits */
...
static SDL_bool GetTextureForMemoryFrame(AVFrame* frame, SDL_Texture** texture)
{
    ...
    SDL_PixelFormatEnum frame_format = GetTextureFormat(static_cast<AVPixelFormat>(frame->format));

    if (*texture) {
        SDL_PropertiesID props = SDL_GetTextureProperties(*texture);
        texture_format = (SDL_PixelFormatEnum)SDL_GetNumberProperty(
    ...
    switch (frame_format) {
        case SDL_PIXELFORMAT_UNKNOWN: {
            ...
            struct SwsContextContainer* sws_container =
                (struct SwsContextContainer*)SDL_GetProperty(props, SWS_CONTEXT_CONTAINER_PROPERTY,
                                                             NULL);
            if (!sws_container) {
                ...
                SDL_SetPropertyWithCleanup(props, SWS_CONTEXT_CONTAINER_PROPERTY, sws_container,
                                           FreeSwsContextContainer, NULL);
            }
            sws_container->context =
                sws_getCachedContext(sws_container->context, frame->width, frame->height,
                                     static_cast<AVPixelFormat>(frame->format), frame->width,
                                     frame->height, AV_PIX_FMT_BGRA, SWS_POINT, NULL, NULL, NULL);
            if (sws_container->context) {
                uint8_t* pixels[4];
                int pitch[4];
        ...
        case SDL_PIXELFORMAT_IYUV:
            if (frame->linesize[0] > 0 && frame->linesize[1] > 0 && frame->linesize[2] > 0) {
                ...
                SDL_UpdateYUVTexture(
                    *texture, NULL, frame->data[0] + frame->linesize[0] * (frame->height - 1),
                    -frame->linesize[0],
...
static SDL_bool GetOESTextureForDRMFrame(AVFrame* frame, SDL_Texture** texture)
{
    ...
    props =
        CreateVideoTextureProperties(frame, SDL_PIXELFORMAT_EXTERNAL_OES, SDL_TEXTUREACCESS_STATIC);
    *texture = SDL_CreateTextureWithProperties(renderer, props);
    SDL_DestroyProperties(props);
    if (!*texture) {
        return SDL_FALSE;
    ...
    colorspace = (SDL_Colorspace)SDL_GetNumberProperty(props, SDL_PROP_TEXTURE_COLORSPACE_NUMBER,
...
static SDL_bool GetTextureForDRMFrame(AVFrame* frame, SDL_Texture** texture)
{
    ...
    if (format_modifier != DRM_FORMAT_MOD_INVALID && format_modifier != DRM_FORMAT_MOD_LINEAR) {
    ...
    *texture = SDL_CreateTextureWithProperties(renderer, props);
    ...
    props = SDL_GetTextureProperties(*texture);
    textures[0] =
        (GLuint)SDL_GetNumberProperty(props, SDL_PROP_TEXTURE_OPENGLES2_TEXTURE_NUMBER, 0);
    textures[1] =
        (GLuint)SDL_GetNumberProperty(props, SDL_PROP_TEXTURE_OPENGLES2_TEXTURE_UV_NUMBER, 0);
    ...
    for (i = 0; i < desc->nb_layers; ++i) {
        ...
        for (j = 0; j < layer->nb_planes; ++j) {
            ...
            EGLImage image =
                eglCreateImage(display, EGL_NO_CONTEXT, EGL_LINUX_DMA_BUF_EXT, NULL, attr);
            if (image == EGL_NO_IMAGE) {
                SDL_Log("Couldn't create image: %d\n", glGetError());
                return SDL_FALSE;
            }
...
static SDL_bool GetTextureForD3D11Frame(AVFrame* frame, SDL_Texture** texture)
{
    ...
    if (!*texture || texture_width != frames->width || texture_height != frames->height) {
        ...
        SDL_PropertiesID props =
            CreateVideoTextureProperties(frame, SDL_PIXELFORMAT_UNKNOWN, SDL_TEXTUREACCESS_STATIC);
        *texture = SDL_CreateTextureWithProperties(renderer, props);
        SDL_DestroyProperties(props);
        if (!*texture) {
            return SDL_FALSE;
    ...
    ID3D11Resource* dx11_resource = static_cast<ID3D11Resource*>(SDL_GetProperty(
        SDL_GetTextureProperties(*texture), SDL_PROP_TEXTURE_D3D11_TEXTURE_POINTER, NULL));
    if (!dx11_resource) {
        SDL_SetError("Couldn't get texture ID3D11Resource interface");
        return SDL_FALSE;
    }
    d3d11_context->CopySubresourceRegion(dx11_resource, 0, 0, 0, 0, (ID3D11Resource*)pTexture,
                                         iSliceIndex, NULL);
...
{
    if (audio) {
        SDL_AudioSpec spec = {GetAudioFormat(static_cast<AVSampleFormat>(frame->format)),
                              frame->ch_layout.nb_channels, frame->sample_rate};
        SDL_SetAudioStreamFormat(audio, &spec, NULL);

        if (frame->ch_layout.nb_channels > 1 &&
            IsPlanarAudioFormat(static_cast<AVSampleFormat>(frame->format))) {
...
{
    static const char* options[] = {"[--verbose]",
                                    "[--sprites N]",
                                    "[--audio-codec codec]",
                                    "[--video-codec codec]",
                                    "[--software]",
...
int main(int argc, char* argv[])
{
    ...
    if (video_stream >= 0) {
        if (video_codec_name) {
            ...
            if (!video_codec) {
                SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Couldn't find codec '%s'",
                             video_codec_name);
                return_code = 4;
                goto quit;
            }
